...
- 每个工具窗口右上角的「参数配置」按钮可调整爬取行为参数（超时、等待间隔、停止阈值等），修改后的参数会保留到下次打开。
...
可通过「参数配置」调整：每个关键词最多视频数（默认 5000）、API 搜索每页条数、视频详情每批条数、每个视频评论最多输出条数。
...
可通过「参数配置」调整：每个作者最多视频/Shorts 数（默认 5000）、页面加载超时、滚动间隔、无新内容停止阈值、每次滚动像素、Posts 最大滚动次数（默认 200）。
...
可通过「参数配置」调整：每个关键词最多视频数（默认 1000）、每个关键词最多检查候选数（默认 3000）、搜索滚动间隔、最大搜索滚动次数、无新内容停止阈值、每个视频评论最多输出条数。
...
可通过「参数配置」调整：切片跨度（默认 7 天）、搜索页加载超时、滚动等待范围、无新内容停止阈值、每个时间切片最大滚动次数（默认 200）。
...
可通过「参数配置」调整：页面加载超时、每条推文评论最多输出条数。
