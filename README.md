...
规则：
- `Videos` 和 `Shorts` 输出内容只取标题，并在标题后追加 `[视频]`。
- `Posts` 会尽量保留完整文本。
- `Posts` 中遇到图片会追加 `[图片]`。
- 视频类作品使用 API 时会尽量写入浏览量、评论数、点赞数；浏览器 fallback 时只写入页面上稳定可识别的浏览量，其它识别不到的指标留空。
- Posts 指标会从卡片文本、按钮、`aria-label`、`title` 和页面数据中尽量识别；识别不到的浏览量、评论数、点赞数会留空。
...
脚本内置：每采集并写入 5 条作品随机等待 20 到 60 秒，避免访问过快。
...
脚本内置：每写入 10 条作品就保存一次，并随机等待 10 到 30 秒，避免访问过快。
