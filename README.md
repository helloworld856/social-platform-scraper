...
- AIGC 判断工具需要 OpenAI 协议兼容 AI 提供商配置。
...
AIGC 判断工具支持两种配置方式：
- 在主界面的“全局配置”中填写 `AI 服务地址`、`AI API Key`、`AI 模型`
- 或在项目根目录配置 `.env`
...
同时兼容旧变量名：`DEEPSEEK_API_KEY`、`DEEPSEEK_BASE_URL`、`DEEPSEEK_MODEL_NAME`、`API_KEY`、`BASE_URL`、`MODEL_NAME`。
...
- `src/judge_aigc/`：AIGC 判断引擎（LangGraph + OpenAI 协议兼容 AI）。
...
用途：围绕同一款游戏，在不同平台、不同语言的搜索轨道上，对「官方名 / 大名」和多组「小名 / 别名 / 玩法词」做可观察搜索覆盖实验。

核心口径：
- 每个游戏由多个 `track` 组成，`track = platform + language + baseline_query + keyword_groups`。
- `baseline_query` 通常放官方名或主名，`keyword_groups` 用于放需要对比的别名组合。
- 报告里的 `Result Count` 表示唯一内容 ID 数，不再是原始链接数。
- 这是**可观察搜索结果覆盖**，不是平台全量内容覆盖率。

适合的配置方式：
- 英文和日文分开建 track，不要混在同一个关键词组里。
- 不同平台单独建 track，因为 TikTok、YouTube、X/Twitter 的有效搜索词往往不同。
- 同一组里的多个关键词会按内容 ID 做并集聚合，用来观察组合召回。

桌面工具使用：
- 工具名：`关键词可观察搜索覆盖实验`
- 左侧维护游戏，中间维护该游戏下的 track，右侧编辑当前 track 的平台、语言、基准词和关键词组。
- 当所选平台和实验配置中的 track 完全不匹配时，工具会直接报错，避免空跑。
- 当 track 中包含 `youtube` 时，才强制要求填写 YouTube API Key。

CLI 使用：

```bash
python -m src.tools.calibration --config config/calibration_config.json --output output/calibration
```

默认样例配置见 [config/calibration_config.json](config/calibration_config.json)，可以直接按 `games[].tracks[]` 结构扩展。
...
output/calibration/<run_id>/
  ...
  raw/<game>/<platform>/<language>/baseline.json
  raw/<game>/<platform>/<language>/group_01.json
...
用途：按关键词和日期范围搜索 YouTube 视频，导出基础指标，并可选补充长短视频类型与快照任务。
...
输出字段包括：标题、视频类型、时长、播放量、点赞数、评论数、作者粉丝数、视频简介、发布时间、视频链接、作者主页链接。
...
两阶段判断：先用本地关键词和 Unicode 范围检测快速筛选，未确定的标题再发送到 OpenAI 协议兼容 AI 做最终判断。
