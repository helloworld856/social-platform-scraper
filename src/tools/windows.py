...
class CalibrationToolWindow(SimpleToolWindow):
    ...
    def __init__(self) -> None:
        super().__init__(
            "关键词覆盖率校准",
            [
                FieldSpec("config_mode", "配置模式", kind="combo", options=("可视化配置 (单游戏)", "使用 JSON 配置文件"), default="可视化配置 (单游戏)"),
                ...
                # JSON 模式专属字段
                FieldSpec("config_path", "JSON 配置文件路径", kind="text_or_file", default="config/calibration_config.json"),
        ...
        # 联动显隐控制
        self.bind_field_visibility("config_mode", "可视化配置 (单游戏)", [
            "game_name", "baseline_query", "keyword_groups", "days",
            "youtube_api_keys", "youtube_max_results", "tiktok_max_videos", "x_max_scrolls", "cdp_url"
        ])
        self.bind_field_visibility("config_mode", "使用 JSON 配置文件", ["config_path"])
    ...
    def validate_values(self, values):
        import os
        if values["config_mode"] == "使用 JSON 配置文件":
