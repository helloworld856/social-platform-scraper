...
class ZendriverTikTokApi:
    ...
    async def _mark_session_invalid(self, session):
        session.is_valid = False
        try:
            if session.owns_tab and session.tab and not session.tab.closed:
    ...
    async def _create_session(
        ...
        try:
            ...
            try:
                ...
                self.logger.debug(f"Could not enable network tracking for session tab: {e}")
            ...
            try:
                await tab.wait_for_ready_state(
                    until="interactive", timeout=max(timeout // 1000, 10)
                )
            except TimeoutError:
                self.logger.warning(
                    "Timed out waiting for TikTok API session tab readiness; continuing with current tab state."
                )
            await tab.send(cdp.input_.dispatch_mouse_event(
            ...
            if tab is not None and existing_tab is None:
                try:
                    await tab.close()
                except Exception:
                    pass
        ...
        **kwargs,
        ...
        for index in range(num_sessions):
            await self._create_session(
                ...
                timeout=timeout,
                existing_tab=existing_tab if index == 0 else None,
            )
            ...
            if self.browser is not None and hasattr(self, "_creation_params"):
                ...
                try:
                    ...
                    await self._create_session(
                        ...
                        timeout=p["timeout"],
                        existing_tab=p.get("existing_tab"),
                    )
                    self.logger.info(
    ...
    def generate_js_fetch(self, method: str, url: str, headers: dict) -> str:
        ...
        return (
            f"(async () => {{"
            f"  const resp = await fetch({url_js}, {{ method: {method_js}, headers: {headers_js} }});"
