...
class PyTok:
    ...
    async def __aenter__(self):
        ...
        # Navigate to TikTok. Some TikTok resources can hang or load slowly; do not
        # fail startup only because the page did not reach the complete state.
        await self._page.send(cdp.page.navigate('https://www.tiktok.com'))
        try:
            async with asyncio.timeout(30):
                await self._page.wait_for_ready_state(until='interactive', timeout=31)
        except TimeoutError:
            self.logger.warning("Timed out waiting for TikTok page readiness; continuing with current browser state.")
