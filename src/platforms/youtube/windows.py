...
class YouTubeKeywordWindow(SimpleToolWindow):
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            [
                ...
                FieldSpec("auto_snapshot_3d", "自动生成3日快照？", kind="combo", options=("是", "否"), default="否"),
                FieldSpec("auto_snapshot_7d", "自动生成7日快照？", kind="combo", options=("是", "否"), default="否"),
    ...
    def run_task(self, values, log_callback, finish_callback, stop_event, pause_event):
        ...
        return run_youtube_keyword_pro(
            ...
            values.get("auto_snapshot_3d", "否"),
            values.get("auto_snapshot_7d", "否"),
            log_callback,
            finish_callback,
            stop_event,
            config=config,
...
class YouTubeKeywordProWindow(YouTubeKeywordWindow):
    tool_id = "youtube_keyword_mining_pro"

    def __init__(self) -> None:
        super().__init__()
        self.setWindowTitle("YouTube 关键词搜索 Pro")
        self.widgets["auto_snapshot_3d"].setCurrentText("是")
        self.widgets["auto_snapshot_7d"].setCurrentText("是")
