...
class FacebookProfileWorksWindow(SimpleToolWindow):
    ...
    def tool_config_params(self):
        return [
            ConfigParam("max_works", "每个作者最大作品数", kind="int", default=5000, minimum=1, maximum=100000),
            ConfigParam("page_load_timeout", "页面加载超时(毫秒)", kind="int", default=45000, minimum=10000, maximum=120000, step=1000),
            ConfigParam("scroll_delay", "滚动间隔(秒)", kind="float", default=2.5, minimum=0.5, maximum=10.0, step=0.1, decimals=1),
            ...
            ConfigParam("max_scrolls", "每个页面最大滚动次数", kind="int", default=200, minimum=1, maximum=5000),
            ConfigParam("save_batch_size", "每N条保存一次", kind="int", default=10, minimum=1, maximum=100),
            ConfigParam("cooldown_min", "批量等待最小(秒)", kind="float", default=9.0, minimum=0.0, maximum=60.0, step=1.0, decimals=1),
            ConfigParam("cooldown_max", "批量等待最大(秒)", kind="float", default=22.0, minimum=0.0, maximum=120.0, step=1.0, decimals=1),
        ]
